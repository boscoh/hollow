...
default_fname = os.path.join(module_dir, "hollow.txt")
...
def find_atom(atoms, chain, res_num, atom_type):
    ...
    for atom in atoms:
        if (
            ...
            return atom
    raise ValueError("Can't find '%s' atom %s of res %d" % (chain, atom_type, res_num))
...
def make_hollow_spheres(
    ...
    if not constraint_file:
        ...
        if constraints.type == "sphere":
            atom1 = find_atom(
                atoms, constraints.chain1, constraints.res_num1, constraints.atom1
            )
            ...
            atom2 = find_atom(
                atoms, constraints.chain2, constraints.res_num2, constraints.atom2
            )
            ...
            width = 2.0 * grid_spacing + 2.0 * math.sqrt(
    ...
    # Make the grid
    n_point = 1.0 * width / grid_spacing
    print(
        "Setting up grid: %d x %d x %d, spacing %.3f, width %.1f"
    ...
    print(
        "Excluding protein bulk from grid with %.1f angstrom probe"
        % size_interior_probe
    )
    timer.start()
    ...
    if is_calculate_asa_shell:
        ...
        print(
            "Excluding surface shell from grid with %.1f angstrom probe"
            % size_surface_probe
        )
        timer.start()
