...
from . import molecule, polymer, vector3d
...
def _splice_to_c_terminus(protein, fragment, j, k):
    ...
    rot = vector3d.Superposition3(ref[0], ref[1], ref[2], mov[0], mov[1], mov[2])

    insert = fragment.extract_polymer(j, k)
    insert.transform(rot)

    protein.insert_polymer(last + 1, insert)
...
def _splice_to_n_terminus(protein, fragment, j, k):
    ...
    protein.insert_polymer(0, insert)


def _splice(protein, i, fragment, j, k):
    insert = fragment.extract_polymer(0, fragment.n_residue())

    if i > 0:
        ref = _get_c_peptide(protein, i - 1)
        mov = _get_n_peptide(insert, j)

        rot1 = vector3d.Superposition3(ref[0], ref[1], ref[2], mov[0], mov[1], mov[2])
    ...
    if i < protein.n_residue():
        ...
        rot2 = vector3d.Superposition3(ref[0], ref[1], ref[2], mov[0], mov[1], mov[2])
...
def calculate_chi(residue, j):
    ...
    raise ValueError("No Chi%d angle for res %d" % (j, residue.i))
...
class Protein(polymer.Polymer):
  ...
  """
    ...
    def chi(self, i, j):
        ...
        if res.type in _chi_topology:
            if j < len(_chi_topology[res.type]):
                p = [
                    res.atom(atom_type).pos for atom_type in _chi_topology[res.type][j]
                ]
                angle = vector3d.pos_dihedral(p[0], p[1], p[2], p[3])
                return vector3d.normalize_angle(angle)
    ...
    def set_chi(self, i, j, chi):
        ...
        p = [res.atom(atom_type).pos for atom_type in _chi_topology[res.type][j]]
    ...
    def set_phi(self, i, phi):
        ...
        pos_list = [
            res.atom(type).pos for type in backbone_atom_types if res.has_atom(type)
        ]
    ...
    def charge_c_end(self):
        ...
        if not self.residue(last).has_atom("OXT"):
            ...
            terminal_oxygen.pos = vector3d.RotatedPos(
                180 * vector3d.DEG2RAD,
    ...
    def charge_n_end(self):
        if not self.residue(0).has_atom("2H"):
            ...
            terminal_hydrogen.pos = vector3d.RotatedPos(
                120 * vector3d.DEG2RAD,
    ...
    def mutate(self, i, res_type):
        ...
        backbone_atom_types = ["C", "O", "H", "N", "CA", "H", "H2", "H3", "OXT", "HXT"]
